"""Local CORS proxy for HLS streams.
...
URL scheme (path-based):
    http://127.0.0.1:{PORT}/https://cdn.example.com/path/manifest.m3u8
"""
    ...
    def _proxy(self, method: str):
        ...
        try:
            ...
            if "mpegurl" in ct.lower() or target.endswith(".m3u8"):
                body = _rewrite_m3u8(
                    body.decode("utf-8", errors="replace"), target
                ).encode("utf-8")
...
def _rewrite_m3u8(content: str, manifest_url: str) -> str:
...
def start_proxy(port: int = PROXY_PORT) -> int:
    """Start the CORS proxy on *port* in a daemon thread.
